...
int startFinish(void)
{
    ...
    // BACKGROUND
    Texture2D background = LoadTexture("./resource3/fin_space.png");

    // PLAYER
    Texture2D playerTexture = LoadTexture("./resource3/player.png");
    float playerScaleX = 5.0f;
    float playerScaleY = 5.0f;
    Rectangle player = {160, SCREEN_HEIGHT - playerTexture.height * playerScaleY, 40, 40};

    // CREATE BUTTON
    Button button = CreateCenteredButton("./resource3/bttn.png", 0.2f, 0.2f, SCREEN_HEIGHT - 100, (Color){182, 223, 240, 255});
    ...
    Texture2D pressETexture = LoadTexture("./resource3/hint_E.png");
